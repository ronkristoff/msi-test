...
export const workspaceNameSchema = z.object({
  name: z.string().trim().min(1, "Please enter a workspace name").max(100, "Workspace name must be under 100 characters"),
});
...
export const workspaceSettingsSchema = z.object({
  name: z.string().trim().min(1, "Workspace name is required").max(100, "Workspace name must be under 100 characters"),
});
