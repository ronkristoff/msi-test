...
describe("buildStoryGenerationPrompt", () => {
  it("returns undefined when both inputs are null", () => {
    expect(buildStoryGenerationPrompt(null, null)).toBeUndefined();
  });

  it("returns undefined when ragText is empty and bmadContext is null", () => {
    expect(buildStoryGenerationPrompt("", null)).toBeUndefined();
  });
  ...
  it("includes STORY_GENERATION_PROMPT and RAG header when only ragText provided", () => {
    const result = buildStoryGenerationPrompt("some code context", null);
    expect(result).toBeDefined();
    expect(result!).toContain(STORY_GENERATION_PROMPT);
    expect(result!).toContain("## Retrieved Codebase Context");
    expect(result!).toContain("some code context");
  });

  it("places STORY_GENERATION_PROMPT before the RAG context block", () => {
    const result = buildStoryGenerationPrompt("RAG_BODY", null);
    expect(result).toBeDefined();
    const promptIdx = result!.indexOf(STORY_GENERATION_PROMPT);
  ...
  it("includes BMAD section when only bmadContext provided", () => {
    const result = buildStoryGenerationPrompt(null, bmadContext);
    expect(result).toBeDefined();
    expect(result!).toContain(STORY_GENERATION_PROMPT);
  ...
  it("includes both RAG and BMAD sections when both provided", () => {
    const result = buildStoryGenerationPrompt("rag text", bmadContext);
    expect(result).toBeDefined();
    expect(result!).toContain("## Retrieved Codebase Context");
    expect(result!).toContain("## BMAD Project Context");
  ...
  it("places STORY_GENERATION_PROMPT before both context sections", () => {
    const result = buildStoryGenerationPrompt("rag text", bmadContext);
    const promptIdx = result!.indexOf(STORY_GENERATION_PROMPT);
  ...
  it("truncates ragText longer than CHAT_RAG_MAX_CONTEXT_CHARS with marker", () => {
    ...
    const result = buildStoryGenerationPrompt(oversized, null);
    expect(result).toBeDefined();
  ...
  it("does NOT truncate ragText exactly at the boundary", () => {
    ...
    const result = buildStoryGenerationPrompt(exact, null);
    expect(result).toBeDefined();
  ...
  it("omits BMAD section when bmadContext has all empty arrays", () => {
    ...
    const result = buildStoryGenerationPrompt("rag text", emptyBmad);
    expect(result).toBeDefined();
    expect(result!).toContain("## Retrieved Codebase Context");
  ...
  it("omits BMAD data section when bmadContext is null but ragText present", () => {
    const result = buildStoryGenerationPrompt("rag text", null);
    expect(result).toBeDefined();
    expect(result!).toContain("## Retrieved Codebase Context");
    expect(result!).not.toContain("### ADRs");
    expect(result!).not.toContain("### Conventions");
  ...
  it("omits specific BMAD subsections that are empty", () => {
    ...
    const result = buildStoryGenerationPrompt(null, partial);
    expect(result).toBeDefined();
    expect(result!).toContain("## BMAD Project Context");
